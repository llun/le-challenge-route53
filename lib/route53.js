...
exports.getZoneIDByName = DNSName => {
  ...
  return route53.listHostedZones().promise()
    .then(result => {
      const zone = result.HostedZones.find(item => `${DNSName}.`.includes(item.Name))
      if (!zone) return Promise.reject(new Error('Zone not found'))
      return zone.Id
    })
};

exports.getChange = Id => {
  return route53.getChange({ Id }).promise()
}
